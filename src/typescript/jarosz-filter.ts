function boxBlurRows(input: Uint8Array, output: Array<number>, width: number, height: number, radius: number) {
...
function boxBlurColumns(input: Array<number>, output: Uint8Array, width: number, height: number, radius: number) {
	const iarr = 1 / (radius + radius + 1); // convolution matrix value - constant for box blur
...
export default (data: Uint8Array, width:number, height: number, passes: number) => {

...
class PDQHasher {
  ...
  static function boxAlongCols(
	...
	//   var_dump("new", $window_size, $half_window_size, $phase_1_nreps, $phase_2_nreps, $phase_3_nreps, $phase_4_nreps);
	...
	// $temp = [];
	// foreach ($out_image AS $item) {
	// 	$temp = \array_merge($temp, \array_map('round', $item));
	// }
	// var_dump($temp);
  }
  ...
  static function readImageFromFilename($filename, $downsample_first) {
    ...
    if (\in_array($mime, ['image/jpg', 'image/jpeg', 'image/pjpeg'])) {
    ...
    if ($downsample_first) {
	  ...
	  $targetWidth = 512;
	  $targetHeight = 512;
      if ($orig_height > $targetHeight || $orig_width > $targetWidth) {
        $image = imagecreatetruecolor($targetWidth, $targetHeight);
        imagecopyresampled($image, $orig_image, 0, 0, 0, 0, $targetWidth, $targetHeight, $orig_width, $orig_height);
      } else {
        $image = $orig_image;
      }
    } else {
	...
	// $png = \ob_get_contents();
	// \ob_end_clean();
	// exit('<img src="data:image/png;base64,'.base64_encode($png).'" />');
...
protected static function renderLuminance(array $data) {
	...
	// Iterate over each row and pixel
	for ($y = 0; $y < $height; $y++) {
		for ($x = 0; $x < $width; $x++) {
			// Get luminance value (adjust range if needed)
			$luminance = intval($data[$y][$x]);

			// Map luminance to grayscale color
			$grey = \imagecolorallocate($image, $luminance, $luminance, $luminance);

			// Set pixel color
			\imagesetpixel($image, $x, $y, $grey);
		}
	}
  ...
  static function computeDCTAndQualityFromImage(
	...
	// 		$diff += abs($value - $luma_matrix[$row][$col]);
	// 	}
	// }
	// var_dump($num_rows, $num_cols, $window_size_along_rows, $window_size_along_cols, round($diff));
	// exit;
	// self::renderLuminance($luma_matrix);
    $t2 = microtime(true);
    if ($show_timings) {

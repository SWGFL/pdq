...
class pdq {
    ...
    public function run(array|string $files, ?string &$error = null) : array|false {
		...
		foreach ($files AS $item) {
			...
			// // For debug purposes only - store the original image (resized for display)
			// if ($debug) {
			// 	// Create a resized copy for display purposes only
			// 	$output[$i]['original'] = $displayImage;
			// }
			...
			// $grayscaleData = $this->luminance($imageData);
			
			// // Create grayscale image for debug
			// if ($debug) {
			// 	$grayscaleImage = $this->dataToImage($grayscaleData, $width, $height);
			// 	$output[$i]['grayscale'] = $this->imageToSquare($grayscaleImage, 512);
			// 	imagedestroy($grayscaleImage);
			// }
			
			// // Apply Jarosz box blur
			// $blurredData = $this->jaroszFilter($grayscaleData, $width, $height, $this->config['passes']);
			
			// // Create blurred image for debug
			// if ($debug) {
			// 	$blurredImage = $this->dataToImage($blurredData, $width, $height);
			// 	$output[$i]['blurred'] = $this->imageToSquare($blurredImage, 512);
			// 	imagedestroy($blurredImage);
			// }
			
			// // Rescale to block size (64x64 by default)
			// $scaledData = $this->rescale($width, $height, $this->config['block'], $blurredData);
			// $scaledImage = $this->dataToImage($scaledData, $this->config['block'], $this->config['block']);
			// $output[$i]['64'] = $scaledImage;
			
			// // Calculate quality metric
			// $quality = $this->calculateQuality($this->config['block'], $scaledData);
			// $output[$i]['quality'] = $quality;
			
			// // Generate DCT
			// $dctData = $this->dct($scaledData);
			
			// // Generate hash(es)
			// $hashResult = $this->generateHashes($dctData, $this->config['transform']);
			// $output[$i]['pdqhash'] = $hashResult;
			
			// // Record processing time
			// $endTime = \microtime(true);
			// $duration = ($endTime - $startTime) * 1000;
			// $output[$i]['time'] = \round($duration);
			
			// // Free the original image to save memory
			// imagedestroy($image);
		}
     ...
     */
    protected function renderData($data, $width, $height) {
        $image = \imagecreatetruecolor($width, $height);
		for ($y = 0; $y < $height; $y++) {
			for ($x = 0; $x < $width; $x++) {
				$luma = \round($data[$y * $width + $x]);
	 ...
	 */
	protected function rescale(array $data, int $width, int $height, int $block) {
     ...
     */
    protected function calculateQuality(array $data, int $block) {
        $gradient = 0;
     ...
     */
    protected function rotateMatrix($data) {
        $len = \count($data);
        $dim = (int)\sqrt($len);
     ...
     */
    protected function flipMatrix($data) {
        $len = \count($data);
        $dim = (int)\sqrt($len);
        $flipped = [];
     ...
     */
    protected function computeDct($dct) {
     ...
     */
    protected function toHex($bytes) {
        $hex = '';
     ...
     */
    public function renderHash($hexHash) {
        ...
        for ($i = 0; $i < $hexLength; $i += 2) {
            if ($i + 1 < $hexLength) {
                $byte = \hexdec(\substr($hexHash, $i, 2));
                for ($bit = 7; $bit >= 0; $bit--) {
                    $binaryHash[] = ($byte >> $bit) & 1 ? 0 : 255;
                }
            }
        }
        
        // Calculate dimensions - should be 16x16 for a 256-bit hash
        $dim = (int)\sqrt(\count($binaryHash));
        ...
        $reversedBinaryHash = \array_reverse($binaryHash);
        
        $idx = 0;
        for ($y = 0; $y < $dim; $y++) {
            for ($x = 0; $x < $dim; $x++) {
                if ($idx < \count($reversedBinaryHash)) {
                    $color = ($reversedBinaryHash[$idx] == 255) ? $white : $black;
                    imagesetpixel($image, $x, $y, $color);
                    $idx++;
                }
            }
